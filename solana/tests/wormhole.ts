...
describe("Wormhole (Core Bridge)", () => {
  ...
  before("Airdrop SOL", async () => {
    await connection.requestAirdrop(wallet.key(), 1000 * web3.LAMPORTS_PER_SOL).then(async (signature) => {
  ...
  describe("Instructions 2 and 7: Post VAA and Verify Signatures", () => {
    it("Verify Guardian Signature and Post Message", async () => {
      ...
      const messageData = await getPostedVaa(connection, CORE_BRIDGE_ADDRESS, parsed.hash).then(
        (postedVaa) => postedVaa.message
      );
  ...
  describe("Instruction 3: Set Fees", () => {
    it("Set Fees to Arbitrary Amount", async () => {
      const previousFee = await getBridgeInfo(connection, CORE_BRIDGE_ADDRESS).then((info) => info.config.fee);
      ...
      const currentFee = await getBridgeInfo(connection, CORE_BRIDGE_ADDRESS).then((info) => info.config.fee);
    ...
    it("Transfer Fees to Recipient", async () => {
      ...
      const transferFeeTx = await web3.sendAndConfirmTransaction(
        ...
        new web3.Transaction().add(
          createTransferFeesInstruction(CORE_BRIDGE_ADDRESS, wallet.key(), recipient, signedVaa)
        ),
        [wallet.signer()]
      );
  ...
  describe("Instruction 6: Upgrade Guardian Set", () => {
    it("Upgrade Guardian Set to 19 Guardians", async () => {
      ...
      const upgradeTx = await web3.sendAndConfirmTransaction(
        connection,
        new web3.Transaction().add(createUpgradeGuardianSetInstruction(CORE_BRIDGE_ADDRESS, wallet.key(), parsed)),
        [wallet.signer()]
      );
